...
재사용 컵 세척 후 고객에게 재제공하기 전 최종 외관 검사를 수행하는 **실운영 산업용 비전 시스템**의 성능 개선 프로젝트.
...
컵 반환 기계
  ...
  ├── 측면 카메라 (4대)  →  컵 외부 360도 촬영  →  본 프로젝트 대상 (동일 모델)
  └── 하단 카메라 (1대)  →  QR코드 / 레터링 확인 (별도 모델)
```
